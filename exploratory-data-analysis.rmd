...
```{r telecom-vs-crimes, fig.width=10, fig.height=7}
df_merge |>
    group_by(sa2_name) |>
    summarize(
        `No Access to Telecom (%)` = sum(`Households - 2023 - Telecom Access (No access to telecommunication systems)`,
            na.rm = TRUE
        ) /
            sum(`Households - 2023 - Telecom Access (Total stated)`, na.rm = TRUE) * 100,
        total_crime = sum(Victimisations, na.rm = TRUE)
    ) |>
...
```{r crowding-vs-crimes, fig.width=10, fig.height=7}
df_merge |>
    group_by(sa2_name) |>
    summarize(
        `Crowded (%)` =
            sum(`Households - 2023 - Crowding Index (Crowded)`, na.rm = TRUE)
            / sum(`Households - 2023 - Crowding Index (Total stated)`, na.rm = TRUE) * 100,
        `Not Crowded (%)` =
            sum(`Households - 2023 - Crowding Index (Not crowded)`, na.rm = TRUE)
            / sum(`Households - 2023 - Crowding Index (Total stated)`, na.rm = TRUE) * 100,
        total_crime = sum(Victimisations, na.rm = TRUE)
    ) |>
    pivot_longer(
